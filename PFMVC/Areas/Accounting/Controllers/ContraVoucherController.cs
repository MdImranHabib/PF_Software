...
namespace PFMVC.Areas.Accounting.Controllers
{
    public class ContraVoucherController : BaseController
    {
        ...
        public ActionResult ContraIndex()
        {
        ...
        /// <ModifiesBy>Avishek</ModifiesBy>
        /// <ModificationDate>24-Feb-2016</ModificationDate>
        public ActionResult ContraVoucher(int VoucherID = 0)
        {
            //Added By Avishek Date:Jan-19-2016
            int oCode = ((int?)Session["OCode"]) ?? 0;
            if (oCode == 0)
            ...
            if (VoucherID == 0)
            {
                ...
                return PartialView("_ContraVoucher", v);
            }
            ...
            return PartialView("_ContraVoucher", vDetail);
        }
        ...
        public ActionResult ContraVoucher(DateTime TransactionDate, int VoucherID, string Narration, IList<Guid> LedgerID, IList<decimal> Debit, IList<decimal> Credit, IList<string> ChequeNumber)
        {
            //Added By Avishek Date:Jan-19-2016
            int oCode = ((int?)Session["OCode"]) ?? 0;
            if (oCode == 0)
            ...
            if (VoucherID == 0)
            {
                ...
                vEntry.VNumber = initialNo + GetMaxVoucherTypeID(3, oCode, initialNo).ToString().PadLeft(4, '0');
                voucherNumber = vEntry.VNumber;
                vEntry.VoucherID = GetMaxVoucherID();
                VoucherID = vEntry.VoucherID;
                vEntry.VoucherName = "";
                vEntry.VTypeID = 3;  // 3 for contra voucher
                vEntry.OCode = oCode;
                vEntry.EditDate = editDate;
                vEntry.EditUser = editUser;
                vEntry.Narration = Narration;
                vEntry.TransactionDate = TransactionDate;//will be deleted after back dated data entry
                ...
                for (int i = 0; i < LedgerID.Count; i++)
                {
                    if (LedgerID[i] != Guid.Empty)
                    {
                        ...
                        vDetail.VTypeID = 3;  // 3 for contra voucher
                        vDetail.TransactionDate = TransactionDate;//will be deleted after back dated data entry
            ...
            try
            {
                ...
                {
                    unitOfWork.Save();
                    return Json(new { Success = true, Message = "Contra voucher Saved!", VoucherNumber = voucherNumber, VoucherID = VoucherID }, JsonRequestBehavior.DenyGet);
                }
        ...
        {
            return PartialView("_ContraVoucherJQ");
        }
    }
}
