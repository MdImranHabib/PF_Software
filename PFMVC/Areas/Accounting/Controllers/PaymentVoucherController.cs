...
namespace PFMVC.Areas.Accounting.Controllers
{
    public class PaymentVoucherController : BaseController
    {
        ...
        /// <ModifiesBy>Avishek</ModifiesBy>
        /// <ModificationDate>24-Feb-2016</ModificationDate>
        public ActionResult PaymentVoucher(int VoucherID = 0)
        {
            //Added By Avishek Date:Jan-19-2016
            int oCode = ((int?)Session["OCode"]) ?? 0;
            if (oCode == 0)
            ...
            if (VoucherID == 0)
            {
                ...
                return PartialView("_PaymentVoucher", v);
            }
            ...
            return PartialView("_PaymentVoucher", vDetail);
        }
        ...
        [HttpPost]
        public ActionResult PaymentVoucher(DateTime TransactionDate, int VoucherID, string Narration, IList<Guid> LedgerID, IList<decimal> Debit, IList<decimal> Credit, IList<string> ChequeNumber)
        {
            //Added By Avishek Date:Jan-19-2016
            int oCode = ((int?)Session["OCode"]) ?? 0;
            tbl_FinancialAuditLog audit = new tbl_FinancialAuditLog();
            if (oCode == 0)
            {
                return RedirectToAction("Login", "Account", new { area = "" });
            }
            ...
            if (VoucherID == 0)
            {
                ...
                string initialNo = "PV-" + TransactionDate.ToString("yy") + "-" + TransactionDate.ToString("MM") + "-";
                vEntry.VNumber = initialNo + GetMaxVoucherTypeID(1, oCode, initialNo).ToString().PadLeft(4, '0');
                ...
                for (int i = 0; i < LedgerID.Count; i++)
                {
                    if (LedgerID[i] != Guid.Empty)
                    {
                        ...
                        vDetail.OCode = oCode;
                        vDetail.VTypeID = 1;  // 1 for payment voucher
        ...
        {
            return PartialView("_PaymentVoucherJQ");
        }
